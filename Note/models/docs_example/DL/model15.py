...
class Model(nn.Model):
    ...
    def __init__(self, input_dim: int, n_train_samples: int, trained_param, distributions, old_train_data, update_freq, kl_threshold_, kl_threshold: float,
        ...
        # Cosine annealing hyper-parameters
        ...
        self.ema_kl = tf.Variable(0.0, trainable=False, dtype=tf.float32)
        self.ema_initialized = tf.Variable(False, trainable=False, dtype=tf.bool)
    ...
    # ------------------------------------------------------------------
    # EMA Update & Dynamic Lambda Annealing
    # ------------------------------------------------------------------
    def update_ema_kl(self, current_kl: tf.Tensor) -> None:
        """Updates the Exponential Moving Average (EMA) of KL divergence."""
        current_kl = tf.stop_gradient(current_kl)
        new_ema = tf.cond(
            self.ema_initialized,
            lambda: self.ema_decay * self.ema_kl + (1.0 - self.ema_decay) * current_kl,
            lambda: current_kl
        )
        self.ema_kl.assign(new_ema)
        self.ema_initialized.assign(True)

    def update_lambda(self) -> None:
        """
        Updates lambda_param using a hybrid mechanism:
        1. Cosine Decay: Establishes a global decreasing trend from lambda_max to lambda_min.
        2. EMA KL Modifier: Dynamically scales lambda according to current representation drift.
        """
        self.current_step.assign(self.batch_counter)
        step_f = tf.cast(self.current_step, tf.float32)
        
        # 1. Base Cosine Annealing (Global monotonic decrease)
        progress = tf.clip_by_value(step_f / self.total_steps, 0.0, 1.0)
        cosine_decay = 0.5 * (1.0 + tf.math.cos(3.1415926535 * progress))
        base_lambda = self.lambda_min + (self.lambda_max - self.lambda_min) * cosine_decay
        
        # 2. Adaptive Modifier driven by EMA KL
        # If EMA KL > threshold (forgetting increases) -> ratio > 1.0 -> increases lambda
        # If EMA KL <= threshold (knowledge preserved) -> ratio <= 1.0 -> allows lambda to decay naturally
        kl_ratio = self.ema_kl / (self.kl_threshold + 1e-8)
        kl_modifier = tf.clip_by_value(kl_ratio, 0.5, 2.0)  # Bound factor between [0.5, 2.0] to prevent instability
        
        # 3. Combine base cosine decay with adaptive modifier
        new_lambda = base_lambda * kl_modifier
    ...
    def loss_func(self, loss: tf.Tensor) -> tf.Tensor:
        ...
        kl_weight = tf.reduce_mean(kl_per_sample_detached)  # Pure scaling metric (detached)

        # Update EMA of KL and compute dynamic lambda_param
        self.update_ema_kl(kl_weight)
        self.update_lambda()
