...
def Stem(in_channels,dtype='float32'):
    ...
    layers.add(batch_norm(momentum=0.9,epsilon=1e-5,parallel=False,dtype=dtype))
...
def XBlock(in_channels, filters_in, filters_out, group_width, stride=1, dtype='float32'):
    ...
    # conv_1x1_2
    layers.add(conv2d(filters_out,[1,1],use_bias=False,weight_initializer='He',dtype=dtype))
    layers.add(batch_norm(momentum=0.9,epsilon=1e-5,parallel=False,dtype=dtype),save_data=True)
...
def ZBlock(
    ...
    layers.add(batch_norm(layers.output_size,momentum=0.9,epsilon=1e-5,parallel=False,dtype=dtype))
