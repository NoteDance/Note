...
def DenseLayer(input_channels, growth_rate, dtype='float32'):
        ...
        layers.add(batch_normalization(epsilon=1.001e-5,parallel=False,dtype=dtype))
...
def TransitionLayer(input_channels, compression_factor, dtype='float32'):
        layers=Layers()
        layers.add(batch_normalization(input_channels,parallel=False,dtype=dtype))
        layers.add(activation_dict['relu'])
...
class DenseNet169:
    ...
    def build(self):
        ...
        self.layers.add(batch_normalization(epsilon=1.001e-5,parallel=False,dtype=self.dtype))
