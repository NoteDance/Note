...
class block1:
    def __init__(self, in_channels, filters, kernel_size=3, stride=1, conv_shortcut=True, dtype='float32'):
        ...
        if conv_shortcut:
            ...
            self.layers1.add(batch_normalization(epsilon=1.001e-5,parallel=False,dtype=dtype))
        else:
            self.layers1.add(identity(in_channels))
        self.layers2=Layers()
        self.layers2.add(conv2d(filters,[1,1],in_channels,strides=[stride],dtype=dtype))
        self.layers2.add(batch_normalization(epsilon=1.001e-5,parallel=False,dtype=dtype))
...
class ResNet101:
    ...
    def build(self,dtype='float32'):
        ...
        if not self.preact:
            self.layers.add(batch_normalization(epsilon=1.001e-5,parallel=False,dtype=dtype))
        ...
        if self.preact:
            self.layers.add(batch_normalization(self.layers.output_size,epsilon=1.001e-5,parallel=False,dtype=dtype))
            self.layers.add(activation_dict['relu'])
