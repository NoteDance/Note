...
class GPT2:
    ...
    def fp(self, X, hparams=hparams, past=None):
        ...
        if self.flag==0:
            self.wpe = tf.Variable(initializer([hparams.n_ctx, hparams.n_embd], ['normal',0.0,0.01], 'float32'))
            self.wte = tf.Variable(initializer([hparams.n_vocab, hparams.n_embd], ['normal',0.0,0.02], 'float32'))
            Module.param.extend([self.wpe,self.wte])
...
class conv1d:
    ...
    def output(self, x, nf, w_init_stdev=0.02):
        *start, nx = shape_list(x)
        if self.flag==0:
            self.w = tf.Variable(initializer([1, nx, nf], ['normal',0.0,w_init_stdev], 'float32'))
            self.b = tf.Variable(initializer([nf],'zeros','float32'))
            Module.param.extend([self.w,self.b])
            self.flag=1
...
class norm:
    ...
    def output(self, x, axis=-1, epsilon=1e-5):
        ...
        n_state = x.shape[-1]
        if self.flag==0:
            self.g = tf.Variable(tf.ones([n_state]))
            self.b = tf.Variable(tf.zeros([n_state]))
            Module.param.extend([self.g,self.b])
            self.flag=1
