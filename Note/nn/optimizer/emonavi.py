...
class EmoZeal(optimizer.Optimizer):
    r"""EmoZeal optimizer.
    ...
    def update_step(self, gradient, variable, learning_rate):
        if tf.keras.backend.is_sparse(gradient):
            raise RuntimeError(
                'EmoZeal does not support sparse gradients')
...
class EmoZeal_sn(optimizer.Optimizer):
    r"""EmoZeal optimizer.
    ...
    def update_step(self, gradient, variable, learning_rate):
        if tf.keras.backend.is_sparse(gradient):
            raise RuntimeError(
                'EmoZeal_sn does not support sparse gradients')
