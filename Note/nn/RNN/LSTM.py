                                ...
                                if self.pattern=='1n':
                                    for k in range(self.labels_shape[1]):
                                        if k==0:
                                            ...
                                            c[j]=tf.nn.tanh(cx[j]+tf.matmul(self.h[j][k-1],cltm_weight_h[j])+cltm_bias[j])
                                    ...
                                    for k in range(self.data_shape[1]):
                                        if k==0:
                                            ...
                                            c[j]=tf.nn.tanh(cx[j][:][:,k]+cltm_bias[j])
                                            self.C[j].append(i[j]*c[j])
                                            self.h[j].append(o[j]*tf.nn.tanh(self.C[j][k]))
                                        else:
                                            ...
                                            c[j]=tf.nn.tanh(cx[j][:][:,k]+tf.matmul(self.h[j][k-1],cltm_weight_h[j])+cltm_bias[j])
                                            self.C[j].append(f[j]*self.C[j][k-1]+i[j]*c[j])
                                            self.h[j].append(o[j]*tf.nn.tanh(self.C[j][k]))
                                    self.h[j]=tf.stack(self.h[j],axis=1)
                        ...
                        if self.pattern=='1n':
                            for j in range(self.labels_shape[1]):
                                if j==0:
                                    ...
                                    c=tf.nn.tanh(cx+tf.matmul(self.h[j],cltm_weight_h)+cltm_bias)
            ...
            for i in range(epoch):
                ...
                if batch!=None:
                    ...
                    for j in range(batches):
                        ...
                        total_loss=total_loss+batch_loss/batch
