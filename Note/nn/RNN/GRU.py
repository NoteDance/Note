                                ...
                                if self.pattern=='1n':
                                    for k in range(self.labels_shape[1]):
                                        if k==0:
                                            ...
                                            c[j]=tf.nn.tanh(cx[j]+tf.matmul(r[j]*self.h[j][k-1],weight_c_h[j])+bias_c[j])
                        ...
                        if self.pattern=='1n':
                            for j in range(self.labels_shape[1]):
                                if j==0:
                                    ...
                                    c=tf.nn.tanh(cx+tf.matmul(r*self.h[j],weight_c_h)+bias_c)
                ...
                if continue_train==True and self.flag==1:
                    if self.layers!=None:
                        ...
                        for i in range(self.layers):
                            ...
                            self.last_rg_weight_x.append(tf.Variable(self.rg_weight_x[i],name='rg_weight_x{}'.format(i+1)))
                            self.last_rg_weight_h.append(tf.Variable(self.rg_weight_h[i],name='rg_weight_h{}'.format(i+1)))
                            ...
                            self.last_rg_bias.append(tf.Variable(self.ig_bias[i],name='rg_bias{}'.format(i+1)))
                ...
                with tf.name_scope('train_loss'):
                    if self.pattern=='1n':
                        if l2==None:
                            ...
                            if self.layers==None:
                                train_loss=tf.reduce_mean(train_loss+l2/2*(tf.reduce_sum(self.ug_weight_x**2)+tf.reduce_sum(self.ug_weight_h**2)+tf.reduce_sum(self.rg_weight_x**2)+tf.reduce_sum(self.rg_weight_h**2)+tf.reduce_sum(self.weight_c_x**2)+tf.reduce_sum(self.weight_c_h**2)+tf.reduce_sum(self.weight_o**2)))
                            else:
                                train_loss=tf.reduce_mean(train_loss+l2/2*(sum([tf.reduce_sum(x**2) for x in self.ug_weight_x])+sum([tf.reduce_sum(x**2) for x in self.ug_weight_h])+sum([tf.reduce_sum(x**2) for x in self.rg_weight_x])+sum([tf.reduce_sum(x**2) for x in self.rg_weight_h])+sum([tf.reduce_sum(x**2) for x in self.weight_c_x])+sum([tf.reduce_sum(x**2) for x in self.weight_c_h])+sum([tf.reduce_sum(x**2) for x in self.weight_o])))
                    elif self.pattern=='n1' or self.predicate==True:
                        if self.pattern=='n1':
                            if l2==None:
