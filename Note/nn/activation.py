...
def activation_conv(data,weight,activation_func,strides,padding,data_format,dilations,conv_func,bias=None):
    if bias is None:
        if activation_func=='sigmoid':
            ...
            return tf.nn.leaky_relu(conv_func(data,weight,strides,padding=padding,data_format=data_format,dilations=dilations),activation_func[1])
        elif type(activation_func)==list and activation_func[0]=='gelu':
            return tf.nn.gelu(conv_func(data,weight,strides,padding=padding,data_format=data_format,dilations=dilations),activation_func[1])
        else:
        ...
        if activation_func=='sigmoid':
            ...
            return tf.nn.leaky_relu(conv_func(data,weight,strides,padding=padding,data_format=data_format,dilations=dilations)+bias,activation_func[1])
        elif type(activation_func)==list and activation_func[0]=='gelu':
            return tf.nn.gelu(conv_func(data,weight,strides,padding=padding,data_format=data_format,dilations=dilations)+bias,activation_func[1])
        else:
            return conv_func(data,weight,strides,padding=padding,data_format=data_format,dilations=dilations)+bias


def activation_conv_transpose(data,weight,output_shape,activation_func,strides,padding,data_format,dilations,conv_func,bias=None):
    if bias is None:
        if activation_func=='sigmoid':
            ...
            return tf.nn.leaky_relu(conv_func(data,weight,output_shape,strides,padding=padding,data_format=data_format,dilations=dilations),activation_func[1])
        elif type(activation_func)==list and activation_func[0]=='gelu':
            return tf.nn.gelu(conv_func(data,weight,output_shape,strides,padding=padding,data_format=data_format,dilations=dilations),activation_func[1])
        else:
        ...
        if activation_func=='sigmoid':
            ...
            return tf.nn.leaky_relu(conv_func(data,weight,output_shape,strides,padding=padding,data_format=data_format,dilations=dilations)+bias,activation_func[1])
        elif type(activation_func)==list and activation_func[0]=='gelu':
            return tf.nn.gelu(conv_func(data,weight,output_shape,strides,padding=padding,data_format=data_format,dilations=dilations)+bias,activation_func[1])
        else:
