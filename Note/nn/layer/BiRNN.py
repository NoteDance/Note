    ...
    def output(self,inputs):
        ...
        for i in range(len(self.fw_cells)):
            ...
            if i==0: # The first layer uses the original input data
                output_fw,state_fw=tf.scan(scan_fn,inputs,initializer=(tf.zeros([batch_size,32]),tf.zeros([batch_size,32])),swap_memory=True)
            else: # The later layers use the output data of the previous layer
                output_fw,state_fw=tf.scan(scan_fn,outputs_fw[-1],initializer=(tf.zeros([batch_size,32]),tf.zeros([batch_size,32])),swap_memory=True)
            outputs_fw.append(output_fw)
            states_fw.append(state_fw)
            cell=self.bw_cells[i]
            if i==0: # The first layer uses the reversed input data
                output_bw,state_bw=tf.scan(scan_fn,inputs_bw,initializer=(tf.zeros([batch_size,32]),tf.zeros([batch_size,32])),swap_memory=True)
            else: # The later layers use the output data of the previous layer
                output_bw,state_bw=tf.scan(scan_fn,outputs_bw[-1],initializer=(tf.zeros([batch_size,32]),tf.zeros([batch_size,32])),swap_memory=True)
