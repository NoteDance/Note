...
class separable_conv1d: # define a class for separable convolutional layer
    def __init__(self,filters,kernel_size,depth_multiplier,input_size=None,strides=[1],padding='VALID',data_format='NHWC',dilations=None,weight_initializer='Xavier',bias_initializer='zeros',activation=None,use_bias=True,trainable=True,dtype='float32'): # define the constructor method
        ...
        if input_size!=None:
            ...
            self.pointwise_kernel=nn.initializer([1,depthwise_filter[-1]*depthwise_filter[-2],filters],weight_initializer,dtype) # initialize the weight matrix for pointwise convolution
    ...
    def build(self):
        ...
        self.pointwise_kernel=nn.initializer([1,depthwise_filter[-1]*depthwise_filter[-2],self.output_size],self.weight_initializer,self.dtype) # initialize the weight matrix for pointwise convolution
