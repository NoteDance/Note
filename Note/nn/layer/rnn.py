    ...
    def __init__(self,weight_shape,timestep,weight_func='uniform',bias_func='zero',activation=None,dtype='float64',return_sequence=False,use_bias=True):
        if type(weight_func)==list:
            ...
            if weight_func=='normal':
                ...
                if dtype!=None:
                    self.weight_i=tf.Variable(tf.random.uniform(shape=weight_shape,dtype=dtype))
                    self.weight_s=tf.Variable(tf.random.uniform(shape=[weight_shape[1],weight_shape[1]],dtype=dtype))
                else:
                    self.weight_i=tf.Variable(tf.random.uniform(shape=weight_shape))
                    self.weight_s=tf.Variable(tf.random.uniform(shape=[weight_shape[1],weight_shape[1]]))
