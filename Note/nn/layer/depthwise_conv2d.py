...
class depthwise_conv2d:
    ...
    def output(self,data,strides,padding='VALID',data_format='NHWC',dilations=None):
        if self.use_bias==True:
            return a.activation_conv(data,self.weight,self.activation,strides,padding,data_format,dilations,tf.nn.depthwise_conv2d,self.bias)
        else:
            return a.activation_conv(data,self.weight,self.activation,strides,padding,data_format,dilations,tf.nn.depthwise_conv2d)
