...
class GCN:
    def __init__(self,input_dim,output_dim,weight_initializer='Xavier',bias_initializer='zero',activation=None,dtype='float32',use_bias=True):
    ...
    def output(self,graph,data):
        ...
        degree_matrix_inv_sqrt=tf.linalg.diag(tf.pow(degree_matrix+1e-10,-0.5))
        norm_adj_matrix=tf.matmul(degree_matrix_inv_sqrt,
                                    tf.matmul(adj_matrix,degree_matrix_inv_sqrt))
