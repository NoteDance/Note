...
class conv1d: # define a class for 1D convolutional layer
    def __init__(self,filters,kernel_size,input_size=None,strides=[1],padding='VALID',weight_initializer='Xavier',bias_initializer='zeros',activation=None,data_format='NWC',dilations=None,use_bias=True,dtype='float32'): # define the constructor method
        ...
        if input_size!=None:
            self.weight=i.initializer([kernel_size,input_size,filters],weight_initializer,dtype) # initialize the weight tensor
    ...
    def build(self):
        self.weight=i.initializer([self.kernel_size,self.input_size,self.output_size],self.weight_initializer,self.dtype) # initialize the weight tensor
    ...
    def output(self,data): # define the output method
        if self.use_bias==True: # if use bias is True
            ...
            return a.activation_conv(data,self.weight,self.activation,self.strides,self.padding,self.data_format,self.dilations,tf.nn.conv1d) # return the output of applying activation function to the convolution of data and weight
