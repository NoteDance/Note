...
class conv1d: # define a class for 1D convolutional layer
    ...
    def output(self,data,strides,padding='VALID',data_format='NWC',dilations=None): # define the output method
        if self.use_bias==True: # if use bias is True
            return a.activation_conv(data,self.weight,self.activation,strides,padding,data_format,dilations,tf.nn.conv1d,self.bias) # return the output of applying activation function to the convolution of data and weight, plus bias
        else: # if use bias is False
            return a.activation_conv(data,self.weight,self.activation,strides,padding,data_format,dilations,tf.nn.conv1d) # return the output of applying activation function to the convolution of data and weight
