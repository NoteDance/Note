...
class conv1d: # define a class for 1D convolutional layer
    ...
    def __call__(self,data): # define the output method
        ...
        if self.input_size==None:
            ...
            self.build()
        if self.use_bias==True: # if use bias is True
            return nn.activation_conv(data,self.weight,self.activation,self.strides,self.padding,self.data_format,self.dilations,tf.nn.conv1d,self.bias) # return the output of applying activation function to the convolution of data and weight, plus bias
        else: # if use bias is False
            return nn.activation_conv(data,self.weight,self.activation,self.strides,self.padding,self.data_format,self.dilations,tf.nn.conv1d) # return the output of applying activation function to the convolution of data and weight