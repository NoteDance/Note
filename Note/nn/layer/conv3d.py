...
class conv3d:
    ...
    def output(self,data,strides,padding='VALID',data_format='NHWC',dilations=None):
        if self.activation=='relu':
            return tf.nn.relu(tf.nn.conv3d(data,self.weight,strides=strides,padding=padding,data_format=data_format,dilations=dilations))
        elif self.activation=='sigmoid':
            return tf.nn.sigmoid(tf.nn.conv3d(data,self.weight,strides=strides,padding=padding,data_format=data_format,dilations=dilations))
        elif self.activation=='tanh':
            return tf.nn.tanh(tf.nn.conv3d(data,self.weight,strides=strides,padding=padding,data_format=data_format,dilations=dilations))
        else:
            return tf.nn.conv3d(data,self.weight,strides=strides,padding=padding,data_format=data_format,dilations=dilations)
