...
class SplitBatchNorm(nn.batch_norm):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.9, trainable=True,
                 num_splits=2):
        super().__init__(num_features, epsilon=eps, momentum=momentum, trainable=trainable)
        assert num_splits > 1, 'Should have at least one aux BN layer (num_splits at least 2)'
        self.num_splits = num_splits
        self.aux_bn = []
        for _ in range(num_splits - 1):
            self.aux_bn.append(nn.batch_norm(num_features, epsilon=eps, momentum=momentum, trainable=trainable))
...
def convert_splitbn_model(module, num_splits=2):
    for layer in module.layer_list:
        if isinstance(layer, nn.batch_norm):
            ...
            layer.__init__(dict_['input_size'], dict_['epsilon'], dict_['momentum'], trainable=dict_['trainable'], num_splits=num_splits)
