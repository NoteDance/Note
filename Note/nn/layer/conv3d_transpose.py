...
class conv3d_transpose: # define a class for 3D transposed convolutional layer
    ...
    def output(self,data): # define the output method
        ...
        new_depth = ((depth - 1) * strides[1] + self.kernel_size[0] - 2 * padding[0] + output_padding[0]) # calculate the new number of depth for the output using the formula 
        new_rows = ((rows - 1) * strides[2] + self.kernel_size[1] - 2 * padding[1] + output_padding[1]) # calculate the new number of rows for the output using the formula 
        new_cols = ((cols - 1) * strides[3] + self.kernel_size[2] - 2 * padding[2] + output_padding[2]) # calculate the new number of columns for the output using the formula 
        
        if self.use_bias==True: # if use bias is True
            return a.activation_conv_transpose(data,self.weight,[data.shape[0],new_depth,new_rows,new_cols,self.output_size],self.activation,strides,self.padding,self.data_format,self.dilations,tf.nn.conv3d_transpose,bias=self.bias) # return the output of applying activation function to the transposed convolution of data and weight, plus bias, using output_shape as output shape
        else: # if use bias is False
