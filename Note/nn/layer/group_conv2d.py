...
class group_conv2d: # define a class for group convolutional layer
    ...
    def __call__(self,data): # define the output method
        ...
        for i in range(self.num_groups): # loop over the number of groups
            if self.use_bias==True: # if use bias is True
                output=a.activation_conv(input_groups[i],self.weight[i],self.activation,self.strides,self.padding,self.data_format,self.dilations,tf.nn.conv2d,self.bias[i]) # calculate the output of applying activation function to the convolution of input group and weight tensor, plus bias vector
            else: # if use bias is False
                output=a.activation_conv(input_groups[i],self.weight[i],self.activation,self.strides,self.padding,self.data_format,self.dilations,tf.nn.conv2d) # calculate the output of applying activation function to the convolution of input group and weight tensor
