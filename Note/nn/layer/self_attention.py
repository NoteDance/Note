...
class self_attention: # define a class for self-attention layer
    def __init__(self,weight_shape,a,weight_initializer='Xavier',mask=None,dtype='float32'): # define the constructor method
    ...
    def output(self,data): # define the output method
        ...
        query=tf.reshape(query,shape=[query.shape[0],query.shape[1],self.a,data.shape[2]//self.a]) # reshape the query vector into a tensor of shape [batch_size, timestep, number of heads, head size]
        key=tf.reshape(key,shape=[key.shape[0],key.shape[1],self.a,data.shape[2]//self.a]) # reshape the key vector into a tensor of shape [batch_size, timestep, number of heads, head size]
        value=tf.reshape(value,shape=[value.shape[0],value.shape[1],self.a,data.shape[2]//self.a]) # reshape the value vector into a tensor of shape [batch_size, timestep, number of heads, head size]
        ...
        scores=tf.matmul(query,key,transpose_b=True)/tf.sqrt(data.shape[2]/self.a) # calculate the attention scores by multiplying query and key tensors (transposing key tensor along last two dimensions), and scaling by square root of head size
        if self.mask is not None: # if mask is not None
            scores+=self.mask*-1e9 # add a large negative value to the masked positions in the scores tensor
