...
class kernel:
    def __init__(self,nn,gradient,loss,update_parameter,optimizer,state,state_name,action_name,exploration_space,save_episode=True):
        self.nn=nn
        self.parameter=None
    ...
    def set_up(self,parameter=None,optimizer=None,epsilon=None,discount=None,episode_step=None,pool_size=None,batch=None,update_step=None,lr=None,end_loss=None,init=False):
        if parameter!=None:
    ...
    def learn1(self):
        if len(self.state_pool)<self.batch:
            with tf.GradientTape() as tape:
                loss=self.loss(self.nn,self.state_pool,self.action_pool,self.next_state_pool,self.reward_pool)
            gradient=self.gradient(loss,self.parameter,tape)
            ...
            for state_batch,action_batch,next_state_batch,reward_batch in train_ds:
                with tf.GradientTape() as tape:
                    batch_loss=self.loss(self.nn,state_batch,action_batch,next_state_batch,reward_batch)
