...
class kernel:
    ...
    def data(self,train_data,train_labels,test_data=None,test_labels=None):
        ...
        if self.PO==3:
            self.batch_shape0=int(self.shape0/self.thread)
            self.shape0=self.batch_shape0
    ...
    def data_func(self,data_batch=None,labels_batch=None,batch=None,index1=None,index2=None,j=None,flag=None,t=None):
        if self.PO==3 and t+1!=self.thread and self.shape0%self.thread!=0:
            _index1=self.thread*self.batch_shape0
            _index2=self.batch_shape0-(self.shape0-self.thread*self.batch_shape0)
        elif self.PO==3:
            _index1=t*self.batch_shape0
            _index2=(t+1)*self.batch_shape0
        if flag==None:
            if type(self.train_data)==list:
                if self.PO!=3:
                    ...
                    for i in range(len(self.train_data)):
                        if batch!=1:
                            if t+1!=self.thread and self.shape0%self.thread!=0:
                                data_batch[i]=self.train_data[i][index1:index2]
                            else:
                                data_batch[i]=self.train_data[i][_index1:_index2][index1:index2]
                        else:
                            if t+1!=self.thread and self.shape0%self.thread!=0:
                                data_batch[i]=self.train_data[i][j]
                            else:
                                data_batch[i]=self.train_data[i][_index1:_index2][j]
            else:
                if self.PO!=3:
                    if batch!=1:
                    ...
                    if batch!=1:
                        if t+1!=self.thread and self.shape0%self.thread!=0:
                            ...
                            data_batch=self.train_data[_index1:_index2][index1:index2]
                    else:
                        if t+1!=self.thread and self.shape0%self.thread!=0:
                            data_batch=self.train_data[j]
                        else:
                            data_batch=self.train_data[_index1:_index2][j]
                    ...
                    for i in range(len(self.train_data)):
                        if batch!=1:
                            if t+1!=self.thread and self.shape0%self.thread!=0:
                                ...
                                labels_batch[i]=self.train_labels[i][_index1:_index2][index1:index2]
                        else:
                            if t+1!=self.thread and self.shape0%self.thread!=0:
                                labels_batch[i]=self.train_labels[i][j]
                            else:
                                labels_batch[i]=self.train_labels[i][_index1:_index2][j]
            else:
                if self.PO!=3:
                    if batch!=1:
                    ...
                    if batch!=1:
                        if t+1!=self.thread and self.shape0%self.thread!=0:
                            ...
                            labels_batch=self.train_labels[_index1:_index2][index1:index2]
                    else:
                        if t+1!=self.thread and self.shape0%self.thread!=0:
                            labels_batch=self.train_labels[j] 
                        else:
                            labels_batch=self.train_labels[_index1:_index2][j] 
        else:
            try:
                if type(self.train_data)==list:
                    if self.PO!=3:
                        ...
                        for i in range(len(self.train_data)):
                            if t+1!=self.thread and self.shape0%self.thread!=0:
                                ...
                                data_batch[i]=self.core.concat([self.train_data[i][_index1:_index2][index1:],self.train_data[i][_index1:_index2][:index2]],0)
                else:
                    if self.PO!=3:
                        ...
                        if t+1!=self.thread and self.shape0%self.thread!=0:
                            ...
                            data_batch=self.core.concat([self.train_data[_index1:_index2][index1:],self.train_data[_index1:_index2][:index2]],0)
                if type(self.train_labels)==list:
                    if self.PO!=3:
                        for i in range(len(self.train_data)):
                        ...
                        if t+1!=self.thread and self.shape0%self.thread!=0:
                            ...
                            for i in range(len(self.train_data)):
                                labels_batch[i]=self.core.concat([self.train_labels[i][_index1:_index2][index1:],self.train_labels[i][_index1:_index2][:index2]],0)
                else:
                    if self.PO!=3:
                        ...
                        if t+1!=self.thread and self.shape0%self.thread!=0:
                            ...
                            labels_batch=self.core.concat([self.train_labels[_index1:_index2][index1:],self.train_labels[_index1:_index2][:index2]],0)
            except:
                if type(self.train_data)==list:
                    if self.PO!=3:
                        for i in range(len(self.train_data)):
    ...
    def opt_t(self,data,labels,t):
        try:
            if self.core.DType!=None:
                pass
            output,loss=self.tf_opt_t(data,labels,t)
            return output,loss
        except AttributeError:
            pass
            return
    ...
    def train_(self,_data_batch=None,_labels_batch=None,batch=None,batches=None,test_batch=None,index1=None,index2=None,j=None,t=None):
        ...
        if batch!=None:
            if index1==batches*batch:
                data_batch,labels_batch=self.data_func(_data_batch,_labels_batch,batch,index1,index2,j,True,t)
                try:
            ...
            data_batch,labels_batch=self.data_func(_data_batch,_labels_batch,batch,index1,index2,j,t=t)
            try:
                output,batch_loss=self.opt_t(data_batch,labels_batch,t)
            except:
                while True:
