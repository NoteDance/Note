...
def s2m(path,endsign=None):
    ...
    for i in range(len(sentence_list)):
        if flag1==0:
            ...
            dw='/'.join(dw)
            dw=dw+'/'
            original_divide_list.append(dw)
            dw=''
            flag1=1
            continue
        if flag1==1:
            ...
            translation_sentence_list.append(sentence_list[i])
            flag1=0
    for i in range(len(original_divide_list)):
        word=''
        original_word_list.append([])
        for j in range(len(original_divide_list[i])):
            if original_divide_list[i][j]=='/':
                original_word_list[i].append(word)
                word_lib_original.add(word)
                word=''
                continue
            word=word+original_divide_list[i][j]
    for i in range(len(translation_sentence_list)):
        word=''
        translation_word_list.append([])
        for j in range(len(translation_sentence_list[i])):
            if translation_sentence_list[i][j]==' ' or translation_sentence_list[i][j] in [',','.','?','!'] or j==len(translation_sentence_list[i])-1:
                if j==len(translation_sentence_list[i])-1 and translation_sentence_list[i][j] not in [',','.','?','!']:
                    word=word+translation_sentence_list[i][j]
    ...
    translation_word_list.append([endsign,',','.','?','!'])
    word_lib_translation.add(endsign)
    word_lib_translation.add(',')
    word_lib_translation.add('.')
    word_lib_translation.add('?')
    word_lib_translation.add('!')
