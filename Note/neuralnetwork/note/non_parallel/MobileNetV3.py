...
class MobileNetV3(Model):
    def __init__(
        ...
        self.layers.add(batch_norm_(epsilon=1e-3,momentum=0.999,parallel=False))
        ...
        self.layers.add(conv2d(last_conv_ch,kernel_size=1,padding="SAME",use_bias=False))
        self.layers.add(batch_norm_(epsilon=1e-3,momentum=0.999,parallel=False))
        self.layers.add(self.activation)
        if self.include_top:
            self.layers.add(global_avg_pool2d(keepdims=True))
            self.layers.add(conv2d(last_point_ch,kernel_size=1,padding="SAME",use_bias=True))
            self.layers.add(self.activation)
...
def _se_block(in_channels, filters, se_ratio):
    ...
    layers.add(conv2d(filters,1,padding='SAME'))
...
class _inverted_res_block:
    def __init__(self, in_channels, expansion, filters, kernel_size, stride, se_ratio, activation, block_id):
        self.conv2d1=conv2d(_depth(in_channels * expansion),1,in_channels,padding="SAME",use_bias=False)
        self.batch_normalization1=batch_norm_(self.conv2d1.output_size,epsilon=1e-3,momentum=0.999,parallel=False)
        if stride == 2:
            self.zeropadding2d=zeropadding2d()
        self.depthwiseconv2d=depthwise_conv2d(kernel_size,input_size=self.conv2d1.output_size,strides=stride,use_bias=False,padding="SAME" if stride == 1 else "VALID")
        self.batch_normalization2=batch_norm_(self.depthwiseconv2d.output_size,epsilon=1e-3,momentum=0.999,parallel=False)
        if se_ratio:
            self.layers=_se_block(self.depthwiseconv2d.output_size, _depth(in_channels * expansion), se_ratio)
            self.conv2d2=conv2d(filters,1,self.layers.output_size,padding="SAME",use_bias=False)
        else:
            self.conv2d2=conv2d(filters,1,self.depthwiseconv2d.output_size,padding="SAME",use_bias=False)
        self.batch_normalization3=batch_norm_(self.conv2d2.output_size,epsilon=1e-3,momentum=0.999,parallel=False)
