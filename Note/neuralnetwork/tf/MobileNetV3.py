...
class MobileNetV3:
    ...
    def build(self,dtype='float32'):
        ...
        self.layers.add(nn.batch_norm(epsilon=1e-3,momentum=0.999,dtype=dtype))
...
class _inverted_res_block:
    def __init__(self, in_channels, expansion, filters, kernel_size, stride, se_ratio, activation, block_id, dtype):
        ...
        self.batch_normalization1=nn.batch_norm(self.conv2d1.output_size,epsilon=1e-3,momentum=0.999,dtype=dtype)
        if stride == 2:
        ...
        self.batch_normalization2=nn.batch_norm(self.depthwiseconv2d.output_size,epsilon=1e-3,momentum=0.999,dtype=dtype)
        if se_ratio:
        ...
        self.batch_normalization3=nn.batch_norm(self.conv2d2.output_size,epsilon=1e-3,momentum=0.999,dtype=dtype)
