...
#                        pass
            ...
            if self.shape0%batch!=0:
                ...
                if type(self.train_data)==list:
                    for i in range(len(self.train_data)):
                        data_batch[i]=torch.concat([self.train_data[i][index1:],self.train_data[i][:index2]])
                else:
                    data_batch=torch.concat([self.train_data[index1:],self.train_data[:index2]])
                if type(self.train_labels)==list:
                    for i in range(len(self.train_data)):
                        labels_batch[i]=torch.concat([self.train_labels[i][index1:],self.train_labels[i][:index2]])
                else:
                    labels_batch=torch.concat([self.train_labels[index1:],self.train_labels[:index2]])
                output=self.nn(data_batch)
                batch_loss=self.nn.loss(output,labels_batch)
                self.nn.opt.zero_grad()
                batch_loss.backward()
...
#                        pass
            ...
            output=self.nn(self.train_data)
                ...
                if type(test_data)==list:
                    for i in range(len(test_data)):
                        if type(test_data)==np.ndarray:
                            ...
                            data_batch[i]=torch.concat(test_data[i][index1:],test_data[i][:index2])
                else:
                    if type(test_data)==np.ndarray:
                        data_batch=np.concatenate(test_data[index1:],test_data[:index2])
                    else:
                        data_batch=torch.concat(test_data[index1:],test_data[:index2])
                if type(self.test_labels)==list:
                    for i in range(len(test_labels)):
                        if type(test_labels)==np.ndarray:
                            ...
                            labels_batch[i]=torch.concat(test_labels[i][index1:],test_labels[i][:index2])
                else:
                    if type(test_labels)==np.ndarray:
                        labels_batch=np.concatenate(test_labels[index1:],test_labels[:index2])
                    else:
                        labels_batch=torch.concat(test_labels[index1:],test_labels[:index2])
                if self.thread==None:
                    output=self.nn.fp(data_batch)
                else:
                    output=self.nn.fp(data_batch,t)
